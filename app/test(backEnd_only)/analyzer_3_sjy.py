...
**규칙: 바뀐 조항은 제목 등 '변경 표기'만 하고, 실제 요약 문장은 작성하지 마세요. 찾기는 두 번째까지 하되, 표기는 '첫 번째 바뀐 것만' 하고 복수면 무조건 '등'으로 표기하세요.**
{{
  "law_title": "법령 명칭 (예: 근로기준법)",
  "article_title": "첫 번째 바뀐 조문 제목만 (예: 제53조 연장 근로의 제한). 없으면 빈 문자열",
  "gov_department": "정부 주관 부서 (예: 고용노동부)",
  "related_departments": ["사내 관련 부서 (인사/HR, 재무/회계, 총무 중 선택)"],
  "summary": "변경 표기만: 첫 번째 바뀐 조항만 '제n조 [조문제목]' 형식으로 표기. 2개 이상이면 반드시 '제n조 [제목] 등'으로 표기. 요약 설명은 넣지 마세요.",
  "article_changes": {{
    "added": ["제n조"],
    "deleted": ["제k조"],
    "modified": ["제p조 [조문제목]"]
...
(article_changes: 찾기는 두 번째까지 하되, 표기는 각 항목당 첫 번째만. 2개 이상이면 반드시 '등'으로 표기.)
...
참고(Regex 추출 결과): {dept_hint}"""),
    ...
    def compare_laws(self, old_text: str, new_text: str):
        """이전/이후 법령의 차이점을 '변경 표기'만 수행. 실제 요약·상세 분석은 하지 않음."""
        prompt = ChatPromptTemplate.from_messages([
            ("system", """당신은 기업 경영지원본부 소속의 법령 개정 분석 전문가입니다.
**규칙: 바뀐 조항 제목 등 '변경 표기'만 하세요. 찾기는 두 번째까지 하되, 표기는 '첫 번째만' 하고 복수면 무조건 '등'으로 표기하세요. 개정 요약, Before/After 상세 비교, 기업 조치 항목은 작성하지 마세요.**

아래 형식으로 '조문 변동 현황'만 작성하세요:

### 📂 조문 변동 현황
- **추가**: 제n조 (1개만 표기. 2개 이상이면 '제n조 등'. 없으면 '없음')
- **삭제**: 제k조 (1개만 표기. 2개 이상이면 '제k조 등'. 없으면 '없음')
- **변경**: 제p조 [조문제목] (1개만 표기. 2개 이상이면 '제p조 [제목] 등'. 없으면 '없음')

위 3개 항목만 작성하고, 그 외 개정 요약·취지·Before/After 비교·조치 필요 항목은 쓰지 마세요."""),
            ("human", "이전 규정:\n{old}\n\n현재 규정:\n{new}")
        ])
